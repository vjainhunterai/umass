...
def ML1():
    ...
    #try:
        #env_local = Variable.get("ENV", default_var = "prod")
    #except Exception as e:
        #print(f"{e}")
    def readEncryptedConfig(excelFilePath,env):
        ...
        # Decrypt the data
        df_decrypted = df.map(lambda x: decryptData(str(x), keyPath))  # FIXED: applymap -> map
    ...
    def remove_matching_chq_no_new(df, group_key='Group_ID'):
        df = df.copy()
        df['matching_index'] = df['matching_index'].apply(lambda x: eval(x) if isinstance(x, str) else x)
        exploded = df[['Index_row', 'Chq_No', 'matching_index']].explode('matching_index')
        exploded = exploded.merge(
            df[['Index_row', 'Chq_No']].rename(columns={'Index_row': 'match_idx', 'Chq_No': 'match_Chq_No'}),
            left_on='matching_index', right_on='match_idx', how='left')
        exploded = exploded[exploded['Chq_No'] != exploded['match_Chq_No']]
        filtered = (exploded.groupby('Index_row')['matching_index'].apply(list).reset_index())
        df = df.drop(columns=['matching_index']).merge(filtered, on='Index_row', how='left')
        df['matching_index'] = df['matching_index'].apply(lambda x: x if isinstance(x, list) else [])
        return df

    def process_date_and_assign_group_new(df, ndays, group_key):
        df = df.sort_values([group_key, 'Inv_Date']).copy()
        df['Date_Diff'] = df.groupby(group_key)['Inv_Date'].diff().dt.days.fillna(0).abs()
        df['New_Flag'] = (df['Date_Diff'] > ndays).astype(int)
        df['Local_Group'] = df.groupby(group_key)['New_Flag'].cumsum()
        df['Group_ID'] = (df[group_key].astype('category').cat.codes * 10 ** 6 + df['Local_Group'])
        return df

    # =========================================================================
    # PATCHED: assign_matching_indices_new
    # Replaced self-merge (O(n^2) memory) with sorted window approach (O(n*k))
    # Produces identical output: for each row, a list of Index_row values
    # within the same Group_ID whose Inv_Date is within ndays.
    # =========================================================================
    def assign_matching_indices_new(df, ndays, group_key='Group_ID'):
        df = df.sort_values([group_key, 'Inv_Date']).copy()

        # Pre-build a position lookup: Index_row -> position in df
        positions = df.index.tolist()
        idx_row_to_pos = {row['Index_row']: pos for pos, (_, row) in zip(positions, df.iterrows())}

        # Initialize matching_index as empty lists
        matching_dict = {pos: [] for pos in positions}

        for group_name, group in df.groupby(group_key):
            if len(group) < 2:
                continue

            group_sorted = group.sort_values('Inv_Date')
            inv_dates = group_sorted['Inv_Date'].values  # numpy datetime64 array
            index_rows = group_sorted['Index_row'].values
            group_positions = [idx_row_to_pos[ir] for ir in index_rows]
            n = len(group_sorted)

            for i in range(n):
                matches = []
                # Look forward from i+1 (dates are sorted, so we can break early)
                for j in range(i + 1, n):
                    diff_days = abs((inv_dates[j] - inv_dates[i]) / np.timedelta64(1, 'D'))
                    if diff_days > ndays:
                        break
                    matches.append(index_rows[j])
                # Look backward from i-1
                for j in range(i - 1, -1, -1):
                    diff_days = abs((inv_dates[i] - inv_dates[j]) / np.timedelta64(1, 'D'))
                    if diff_days > ndays:
                        break
                    matches.append(index_rows[j])

                matching_dict[group_positions[i]] = matches

        df['matching_index'] = [matching_dict[pos] for pos in positions]
        df['matching_index'] = df['matching_index'].apply(lambda x: x if isinstance(x, list) else [])
        return df
    ...
    gc.collect()
    del combined_reserve_copy
    del original_full_dataset_reserve_copy
    global group_id_counter

    print("original_rulebased")
    original_rulebased = pd.DataFrame()
    try:
        df = getDataFromDatabase_temorary('random1')
        df = df.dropna(subset=['Chq_No'])
        df = df[df['Chq_No']!='']
        print(df.shape)
        df['Group_ID'] = (
            df[['VENDOR_NAME_ALIAS', 'Pay_Amount', 'Invoice_Amount']]
            .astype(str)
            .agg('-'.join, axis=1)
            .rank(method='dense').astype(int)
        )
        pair_id_counter = count(1)
        group_id_counter = df['Group_ID'].max()+1
        tqdm.pandas()
        # ndays = 5
        print("process_date_and_assign_group")
        df = process_date_and_assign_group_new(df, ndays=5, group_key='Group_ID')
        print("assign_matching_indices")
        df = assign_matching_indices_new(df, ndays=5, group_key='Group_ID')
        #df = df.drop(columns=['Date_Diff', 'New_Group'])
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        df = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        print("remove_matching_chq_no")
        df = remove_matching_chq_no_new(df, group_key='Group_ID')
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        df = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        print("filter_by_levenshtein_distance")
        df = df.groupby('Group_ID', group_keys=False).progress_apply(filter_by_levenshtein_distance)
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        df = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        print("filter_by_numeric_difference")
        df = df.groupby('Group_ID', group_keys=False).progress_apply(filter_by_numeric_difference)
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        original_rulebased = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        if flag_check_number == 1:
            original_rulebased['Chq_No'] = original_rulebased['Chq_No_Old']
            original_rulebased = original_rulebased.drop(columns=['Chq_No_Old'])
        print(datetime.now().strftime("%H:%M:%S"))
        print(f"original_rulebased - {original_rulebased.shape}")
    except Exception as e:
        print(f"{e}")
    gc.collect()
    print("rulebased1")
    print("Date issue - up to 31 days")
    rulebased1 = pd.DataFrame()
    try:
        print(datetime.now().strftime("%H:%M:%S"))
        df = getDataFromDatabase_temorary('random1')
        df = df.dropna(subset=['Chq_No'])
        ...
        pair_id_counter = count(1)
        df['Group_ID'] = (
            df[['VENDOR_NAME_ALIAS', 'Pay_Amount',  'Invoice_Amount', 'Default_Payment_Terms','Suppliers_Invoice_Number']]
            .astype(str)
            .agg('-'.join, axis=1)
            .rank(method='dense').astype(int)
        )
        ...
        print("process_date_and_assign_group")
        df = process_date_and_assign_group_new(df, ndays=31, group_key='Group_ID')
        print("assign_matching_indices")
        df = assign_matching_indices_new(df, ndays=31, group_key='Group_ID')
        ...
        df = remove_matching_chq_no_new(df, group_key='Group_ID')
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        rulebased1 = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        if flag_check_number == 1:
            rulebased1['Chq_No'] = rulebased1['Chq_No_Old']
            rulebased1 = rulebased1.drop(columns=['Chq_No_Old'])
        print(datetime.now().strftime("%H:%M:%S"))
        print(f"rulebased1 - {rulebased1.shape}")
    except Exception as e:
        print(f"{e}")
    gc.collect()

    print("Different VENDOR_ALIAS NAME")
    print("rulebased2")
    rulebased2 = pd.DataFrame()
    try:
        print(datetime.now().strftime("%H:%M:%S"))
        df = getDataFromDatabase_temorary('random1')
        df = df.dropna(subset=['Chq_No'])
        ...
        df['Group_ID'] = (
            df[['Inv_Date','Pay_Amount', 'Quantity','Item_Desc', 'Purchase_Orders', 'Invoice_Amount', 'Suppliers_Invoice_Number']]
            .astype(str)
            .agg('-'.join, axis=1)
            .rank(method='dense').astype(int)
        )
        ...
        print(f"Unique group_id is {df['Group_ID'].nunique()}")

        ndays=0
        print("process_date_and_assign_group")
        df = process_date_and_assign_group_new(df, ndays=1, group_key='Group_ID')
        print("assign_matching_indices")
        ndays=1
        df = assign_matching_indices_new(df, ndays=1, group_key='Group_ID')
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        df = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        #df = df.drop(columns=['Date_Diff', 'New_Group'])
        print("remove_matching_chq_no")
        df = remove_matching_chq_no_new(df, group_key='Group_ID')
        df['matching_index'] = df['matching_index'].apply(lambda x: str(x) if isinstance(x, list) else x)
        rulebased2 = df[df['matching_index'].apply(lambda x: (isinstance(x, list) and len(x) != 0) or (isinstance(x, str) and x.strip() != '[]'))]
        if flag_check_number == 1:
            rulebased2['Chq_No'] = rulebased2['Chq_No_Old']
            rulebased2 = rulebased2.drop(columns=['Chq_No_Old'])
        print(f"rulebased2 - {rulebased2.shape}")
        print(datetime.now().strftime("%H:%M:%S"))
    except Exception as e:
        print(f"{e}")
    ...
    print("START_CLUSTERING")
    def pairwise_clustering(df, index_col = 'Index_row', matching_col = 'matching_index',cluster_col = 'cluster_id' ):
        df_result = df.copy()
        def parse_matches(x):

            return ast.literal_eval(x) if pd.notnull(x) else []
        df_result[matching_col] = df_result[matching_col].apply(parse_matches)
        G = nx.Graph()
        all_indices = sorted(df_result[index_col].unique())
        G.add_nodes_from(all_indices)

        index_to_neighbors = {}
        for i, row in df_result.iterrows():
            current_idx = row[index_col]
            neighbors = sorted(row[matching_col])
            index_to_neighbors[current_idx] = neighbors
        for node, neighbors in index_to_neighbors.items():
            for nb in neighbors:
                G.add_edge(node, nb)
        global forced_pairs
        forced_pairs = []
        ...
        while True:
            single_neighbor_nodes = sorted([n for n in G.nodes if G.degree(n) ==1])
            if not single_neighbor_nodes:
                break
            single_neighbor_nodes.sort()
            for node in single_neighbor_nodes:
                if node in G:
                    neighbor_list = list(G[node])
                    if len(neighbor_list) ==1:
                        partner = neighbor_list[0]
                        forced_pairs.append(tuple(sorted((node,partner))))
                        G.remove_node(node)
                        if partner in G:
                            G.remove_node(partner)
        max_match = nx.max_weight_matching(G,maxcardinality = True)
        matched_pairs = [tuple(sorted(list(edge))) for edge in max_match]
        all_pairs = forced_pairs + matched_pairs
        global cluster_id_map
        cluster_id_map = {}
        current_cluster_id = 1
        for pair in sorted(all_pairs):
            u, v = pair
            cluster_id_map[u] = current_cluster_id
            cluster_id_map[v] = current_cluster_id
            current_cluster_id +=1
        def get_cluster_id(row, idx):
            return cluster_id_map.get(row_idx, None)
        df_result[cluster_col] = df_result[index_col].apply(lambda x: cluster_id_map.get(x,None))
        return df_result

    try:
        original_rulebased['cluster_id']=0
        original_rulebased = pairwise_clustering(original_rulebased)
        print(f"Unique clusters {original_rulebased['cluster_id'].nunique()}")
    except Exception as e:
        print(f"{e}")
    try:
        rulebased1['cluster_id']=0
        rulebased1 = pairwise_clustering(rulebased1)
        print(f"Unique clusters {rulebased1['cluster_id'].nunique()}")
    except Exception as e:
        print(f"{e}")
    try:
        rulebased2['cluster_id']=0
        rulebased2 = pairwise_clustering(rulebased2)
        print(f"Unique clusters {rulebased2['cluster_id'].nunique()}")
    except Exception as e:
        print(f"{e}")

    print(f"Before merging files")


    rulebased7 = pd.DataFrame()
    print(original_rulebased.shape)
    original_rulebased.rename(columns={'cluster_id': 'Original_rulebased_GroupID'}, inplace=True)
    combined =  pd.concat([rulebased7, original_rulebased], ignore_index=True)

    print(rulebased1.shape)
    rulebased1.rename(columns={'cluster_id': 'Rulebased1__GroupID'}, inplace=True)
    combined =  pd.concat([combined, rulebased1], ignore_index=True)

    print(rulebased2.shape)
    rulebased2.rename(columns={'cluster_id': 'Rulebased2_Group_ID'}, inplace=True)
    combined =  pd.concat([combined, rulebased2], ignore_index=True)
    rename_mapping = {
        'Chq_No': 'Check_Number',
    ...
    potential_columns = ['Rulebased7_Group_ID', 'Original_rulebased_GroupID', 'Rulebased1__GroupID', 'Rulebased2_Group_ID']
    columns = [col for col in potential_columns if col in combined.columns]
    combined = combined.dropna(subset=columns, how='all')
    print(f"step_2 - {combined.shape}")
        ...
        for index, row in tqdm(combined.iterrows(), total=len(combined)):
            if pd.isna(combined.at[index, 'Matched_Record_Number']):
                ...
                if pd.notna(value):
                    ...
                    unique_id = get_unique_id(f"{col}_{value}") #
